// graphGenerator.js — port of graphGenerator.ts for Node.js backend.
// Generates the same tree structure as the frontend but without SVG coordinates
// (x/y are not needed for the search algorithm).
'use strict';
...
/**
 * Generates a graph for the given generation and side ('top' or 'bot').
 * Returns { id, nodes: [{id, isFrontier}], edges: [{id, sourceId, targetId, color}] }.
 * x/y fields are omitted — they are only needed for SVG rendering, not search.
 */
function generateGraph(gen, graphId) {
  const nodes   = [];
  const edges   = [];
  const records = []; // { level, pos, parentEdgeColor }
  ...
  for (let level = 0; level <= gen; level++) {
    ...
    for (let pos = 0; pos < count; pos++) {
      ...
      if (level === 1) {
        ...
        edges.push({
          id:       `${graphId}-e${level}:${pos}`,
          sourceId: `${graphId}-0:0`,
          targetId: id,
        ...
        edges.push({
          id:       `${graphId}-e${level}:${pos}`,
          sourceId: `${graphId}-${level - 1}:${parentPos}`,
          targetId: id,
          color:    parentEdgeColor,
        });
      }
